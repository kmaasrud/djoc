...
all:
	...
	for arch in "amd64" "arm64"; do \
		GOOS=linux GOARCH=$$arch go build -ldflags="-s -w -X 'main.VERSION=$(VER)'" -o bin/bins/doctor_linux_$$arch ; \
	...
	for arch in "amd64" "arm"; do \
		GOOS=windows GOARCH=$$arch go build -ldflags="-s -w -X 'main.VERSION=$(VER)'" -o bin/bins/doctor_windows_$$arch.exe ; \
