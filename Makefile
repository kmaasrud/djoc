...
build:
	for arch in "amd64" "arm64"; do \
		GOOS=linux GOARCH=$$arch go build -ldflags="-s -w" -o bin/bins/doctor-$(VER)-linux-$$arch ; \
		cp bin/bins/*linux-$$arch* bin/doctor ; \
		tar -czvf bin/doctor-$(VER)-linux-$$arch.tar.gz bin/doctor ; \
		rm bin/doctor ; \
	done
	for arch in "amd64" "arm64"; do \
		GOOS=darwin GOARCH=$$arch go build -ldflags="-s -w" -o bin/bins/doctor-$(VER)-macOS-$$arch ; \
		cp bin/bins/*macOS-$$arch* bin/doctor ; \
		zip bin/doctor-$(VER)-macOS-$$arch.zip bin/doctor ; \
		rm bin/doctor ; \
	done ; \
	for arch in "amd64" "arm"; do \
		GOOS=windows GOARCH=$$arch go build -ldflags="-s -w" -o bin/bins/doctor-$(VER)-windows-$$arch.exe ; \
